...
class ScrapingPipeline:
    ...
    def __init__(
        ...
        self.llm_enricher = LLMEnricher() if self.enable_llm_enrichment else None
        
        logger.info(
            f"Pipeline initialized: scoring={enable_scoring}, "
            f"dedup={enable_deduplication}, llm={self.enable_llm_enrichment}"
        )
    ...
    def run(
        ...
        try:
            ...
            if self.enable_llm_enrichment and self.llm_enricher:
                logger.info("Stage 5: LLM enrichment")
                ...
                logger.info(f"Enriched {result.llm_enriched_count} jobs with LLM")
            
            # Stage 6: Filter by relevance
            if self.enable_scoring:
                logger.info("Stage 6: Filtering by relevance")
